...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None, val_size=None):
        ...
        if self.classes is None and self.pictures is None:
            ...
            features_filename = (
                os.path.join(features_path, f"{model_type}_features_train.pt")
                if self.train
                else os.path.join(features_path, f"{model_type}_features_test.pt")
            )
            ...
            if os.path.exists(features_filename) :
                ...
                # Retrieval training/eval only uses *_features, not the raw text/image paths.
                if len(self.text) == 0:
